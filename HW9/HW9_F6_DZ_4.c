...
int is_two_same(int size, int a[]) {
    for (int i = 0; i < size; i++) {
        for (int j = i + 1; j < size; j++) {
            if (a[i] == a[j]) {
                return 1; // Если есть два одинаковых элемента, возвращаем 1
            }
        }
    }
    return 0; // Если нет двух одинаковых элементов, возвращаем 0
}

int main() {
    int a[100]; // Максимальный размер массива
    int size = 0;

    printf("Введите элементы массива (для завершения ввода введите любой символ):\n");
